...
router.post('/register', asyncHandler(async (req, res) => {
    ...
    let role = 'notetaker';
    if (Number(userCount) === 0) role = 'admin';

    const caller = await currentUser(req);
    const callerIsAdmin = caller && caller.role === 'admin' && Number(userCount) > 0;
    if (callerIsAdmin && ROLES.includes(req.body.role)) role = req.body.role;

    const hash = bcrypt.hashSync(password, 10);
    const info = await db.prepare('INSERT INTO users (email, password_hash, role) VALUES (?, ?, ?) RETURNING id').run(email, hash, role);
    ...
    if (!callerIsAdmin) await signCookie(res, user);
...
router.post('/login', asyncHandler(async (req, res) => {
    const email = String(req.body.email || '').trim().toLowerCase();
    const password = String(req.body.password || '');
    const row = await db.prepare('SELECT id, email, password_hash, role FROM users WHERE email = ?').get(email);
