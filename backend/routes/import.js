...
router.post('/', requireAuth, requireRole('admin', 'notetaker'), asyncHandler(async (req, res) => {
    const sermons = Array.isArray(req.body.sermons) ? req.body.sermons : [];
    let imported = 0, skipped = 0;

    await db.transaction(async tx => {
        const findExisting = tx.prepare('SELECT id FROM sermons WHERE title = ? AND date = ?');
        const insertSermon = tx.prepare('INSERT INTO sermons (title, date, elapsed, created_by) VALUES (?, ?, ?, ?) RETURNING id');
        ...
        for (const s of sermons) {
            ...
            if (await findExisting.get(title, date)) { skipped++; continue; }
            ...
            for (let idx = 0; idx < paras.length; idx++) {
                const p = paras[idx];
                const pInfo = await insertPara.run(sermonId, idx, Number(p.t) || 0, String(p.lang || 'en'), null, String(p.text || ''));
